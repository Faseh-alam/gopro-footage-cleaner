...
export function PlayerPanel({ c }: { c: ReviewController }) {
  ...
  return (
    <section className="panel-surface flex min-h-0 flex-col">
      ...
      <div className="grid min-h-0 lg:grid-cols-[minmax(0,1fr)_minmax(220px,280px)]">
        <div
          ref={c.playerWrapRef}
          tabIndex={0}
          className="relative min-w-0 bg-black outline-none focus-visible:ring-2 focus-visible:ring-ring"
        >
          <video
            ref={c.videoRef}
            playsInline
        ...
        <div className="flex min-h-0 flex-col border-t border-border lg:border-l lg:border-t-0">
          <div className="flex items-center justify-between gap-2 border-b border-border px-3 py-2">
            ...
            <span className="font-mono text-[10px] text-muted-foreground">latest first</span>
          </div>
          <ul className="max-h-64 min-h-0 flex-1 divide-y divide-border overflow-auto lg:max-h-none lg:min-h-[12rem]">
            {segments.length === 0 && (
            ...
            {[...segments].reverse().map((s, revI) => {
              ...
              return (
                <li key={s.id || i} className="flex items-center justify-between gap-2 px-3 py-2 text-xs">
