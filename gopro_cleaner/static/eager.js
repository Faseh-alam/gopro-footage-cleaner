...
const el = {
  ...
  queueClipsBtn: document.getElementById("queue-clips-btn"),
};
...
// 2. Drastically speed up keyboard navigation by updating classes instead of re-rendering[cite: 1].
function moveTaskSelection(delta) {
  ...
  // FAST DOM UPDATE: Only toggle the active class, don't rebuild the whole list
  const items = Array.from(el.taskList.querySelectorAll(".task-item"));
  items.forEach(item => item.classList.remove("active"));

  const nextActive = items.find(item => item.textContent === chosen);
  if (nextActive) {
  ...
  if (el.taskSelectedHint) {
    el.taskSelectedHint.textContent = `Selected: ${chosen} — Enter assigns pending work`;
  }
  return true;
}
...
function focusTaskSearch() {
  ...
  el.taskSearch.focus();
  el.taskSearch.select(); // Selects the text for rapid replacement
...
// Replace the existing taskSearch keydown listener[cite: 1] with this:
...
el.queueClipsBtn.addEventListener("click", queueClips);
...
async function queueClips() {
  ...
  if (current) {
    const ann = currentAnnotation() || await loadAnnotationForPath(current.path);
    const clips = (ann?.segments || []).filter(s => s.kind === "work").reduce((acc, segment) => acc ? acc + "\n" + `${convertSecondsToTimecode(segment.start)} - ${convertSecondsToTimecode(segment.end)}` : `${convertSecondsToTimecode(segment.start)} - ${convertSecondsToTimecode(segment.end)}`, "");
    const textarea = document.createElement("textarea");
    textarea.value = clips;
    try {
      console.log()
      ...
      el.queueClipsBtn.disabled = false;
    }
  }
}
...
const convertSecondsToTimecode = (seconds) => {
  ...
  const _hours = hours !== 0 ? pad(hours) : '';

  return `${_hours}${pad(minutes)}:${pad(secs)}`;
}